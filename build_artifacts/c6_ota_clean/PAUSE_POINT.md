...
> **Historical investigation record.** Do not use this file as current
> provisioning or release guidance. The supported path is the pinned
> `esp32-p4-micropython-installer`, which verifies the activated C6 ELF identity
> and ESP-NOW service before installing the selected P4 firmware.
...
Use the installer repository and its coordinated manifest. Producer firmware
comes from immutable `firmware-v*` releases; the installer imports all four P4
variants atomically, verifies compatibility with the bundled C6 image, and
packages only pinned, hash-verified files.